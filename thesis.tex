...
\chapter{VISUAL REASONING AND STATISTICAL GRAPHICS}\label{VisualAptitude}
...
\item \citet{hofmann2012graphical} for lineup stimuli
\end{itemize}
...
\section{Methods}
Participants will complete the following tasks (sample pictures included, full stimuli set will be added to the appendix once testing is complete): 
\begin{itemize}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{lineup.summary}\hlopt{$}\hlstd{variable} \hlkwb{<-} \hlkwd{factor}\hlstd{(lineup.summary}\hlopt{$}\hlstd{variable,} \hlkwc{labels}\hlstd{=}\hlkwd{c}\hlstd{(}\hlstr{"Video Game Hrs/Wk"}\hlstd{,} \hlstr{"Card Rotation"}\hlstd{,} \hlstr{"Figure Classification"}\hlstd{,} \hlstr{"Paper Folding"}\hlstd{,} \hlstr{"Visual Search"}\hlstd{))}
\hlstd{lineup.summary}\hlopt{$}\hlstd{variable} \hlkwb{<-} \hlkwd{factor}\hlstd{(lineup.summary}\hlopt{$}\hlstd{variable,} \hlkwc{levels}\hlstd{=}\hlkwd{c}\hlstd{(}\hlstr{"Card Rotation"}\hlstd{,} \hlstr{"Figure Classification"}\hlstd{,} \hlstr{"Paper Folding"}\hlstd{,} \hlstr{"Visual Search"}\hlstd{,} \hlstr{"Video Game Hrs/Wk"}\hlstd{))}

\hlkwd{qplot}\hlstd{(}\hlkwc{data}\hlstd{=lineup.summary,} \hlkwc{x}\hlstd{=value,} \hlkwc{y}\hlstd{=lineup,} \hlkwc{geom}\hlstd{=}\hlstr{"point"}\hlstd{,} \hlkwc{shape}\hlstd{=}\hlkwd{I}\hlstd{(}\hlnum{1}\hlstd{),} \hlkwc{size}\hlstd{=}\hlkwd{I}\hlstd{(}\hlnum{3}\hlstd{))} \hlopt{+} \hlkwd{facet_wrap}\hlstd{(}\hlopt{~}\hlstd{variable,} \hlkwc{scales}\hlstd{=}\hlstr{"free"}\hlstd{,} \hlkwc{ncol}\hlstd{=}\hlnum{3}\hlstd{)} \hlopt{+} \hlkwd{xlab}\hlstd{(}\hlstr{"Value"}\hlstd{)} \hlopt{+} \hlkwd{ylab}\hlstd{(}\hlstr{"Lineup Score (Proportion answered correctly)"}\hlstd{)} \hlopt{+} \hlkwd{geom_smooth}\hlstd{(}\hlkwc{method}\hlstd{=}\hlstr{"lm"}\hlstd{)}
\hlkwd{ggsave}\hlstd{(}\hlkwd{paste}\hlstd{(imgdir,} \hlstr{"fig-VisReasoningAnalysisCts.png"}\hlstd{,} \hlkwc{sep}\hlstd{=}\hlstr{""}\hlstd{),} \hlkwc{width}\hlstd{=}\hlnum{6}\hlstd{,} \hlkwc{height}\hlstd{=}\hlnum{4}\hlstd{)}

\hlstd{lineup.summary.categorical}\hlopt{$}\hlstd{variable} \hlkwb{<-} \hlkwd{factor}\hlstd{(lineup.summary.categorical}\hlopt{$}\hlstd{variable,} \hlkwc{labels}\hlstd{=}\hlkwd{c}\hlstd{(}\hlstr{"Age"}\hlstd{,} \hlstr{"Sex"}\hlstd{,} \hlstr{"Math/Science Research"}\hlstd{,} \hlstr{"Statistics Class"}\hlstd{,} \hlstr{"Calculus 1"}\hlstd{,} \hlstr{"Verbal Skills"}\hlstd{,} \hlstr{"Math Skills"}\hlstd{,} \hlstr{"Art Skills"}\hlstd{,} \hlstr{"AutoCAD Experience"}\hlstd{))}
\hlkwd{qplot}\hlstd{(}\hlkwc{data}\hlstd{=lineup.summary.categorical,} \hlkwc{x}\hlstd{=value,} \hlkwc{y}\hlstd{=lineup,} \hlkwc{geom}\hlstd{=}\hlstr{"point"}\hlstd{,} \hlkwc{shape}\hlstd{=}\hlkwd{I}\hlstd{(}\hlnum{1}\hlstd{),} \hlkwc{size}\hlstd{=}\hlkwd{I}\hlstd{(}\hlnum{3}\hlstd{))} \hlopt{+} \hlkwd{facet_wrap}\hlstd{(}\hlopt{~}\hlstd{variable,} \hlkwc{scales}\hlstd{=}\hlstr{"free"}\hlstd{,} \hlkwc{ncol}\hlstd{=}\hlnum{3}\hlstd{)} \hlopt{+} \hlkwd{xlab}\hlstd{(}\hlstr{"Value"}\hlstd{)} \hlopt{+} \hlkwd{ylab}\hlstd{(}\hlstr{"Lineup Score (Proportion answered correctly)"}\hlstd{)}
\hlkwd{ggsave}\hlstd{(}\hlkwd{paste}\hlstd{(imgdir,} \hlstr{"fig-VisReasoningAnalysisCat.png"}\hlstd{,} \hlkwc{sep}\hlstd{=}\hlstr{""}\hlstd{),} \hlkwc{width}\hlstd{=}\hlnum{6}\hlstd{,} \hlkwc{height}\hlstd{=}\hlnum{6}\hlstd{)}
\end{alltt}
\end{kframe}

{\centering \includegraphics[width=\maxwidth]{../Figure/VisualAptitude/fig-VisReasoningAnalysis1} 
\includegraphics[width=\maxwidth]{../Figure/VisualAptitude/fig-VisReasoningAnalysis2} 
...
Experiment Setup: We will use Amazon Turk to gather data to construct a feature hierarchy for statistical graphics. For instance, do participants notice color before they notice the slope of a line? The experiment will use the lineup protocol with two test plots; one will have a signal associated with a certain feature (i.e. color) and the other will have a signal associated with the comparison feature (i.e. slope). We intend to examine color, slope, presence of outliers, nonlinear trend, and other traits in this experiment. 

Experiment Progress: We are working on quantifying the strength of the signal in each plot, so that we can ensure that the features are of similar strength. 
