...
<<steps,echo=F,include=FALSE, fig.width=4, fig.height=4>>=
library(datasets)
library(ggplot2)
data(faithful)
qplot(waiting, eruptions, geom="point", data=faithful) + xlab("Time Between Eruptions (minutes)") + ylab("Eruption Length (minutes)") + ggtitle("Old Faithful Eruptions") + geom_smooth(method="lm")
@
...
ggplot(data=iris, aes(x=Sepal.Width, group=Species, fill=Species)) + 
  geom_density(position="stack") + theme(legend.position=c(.8, .8)) + xlab("Sepal Width")

# Dotplot 
ggplot(data=iris, aes(x=Sepal.Width, group=Species, fill=Species)) + 
  geom_dotplot(method='histodot', stackgroups=TRUE) + theme(legend.position=c(.8, .8)) + theme(axis.text.y=element_blank()) + ylab("") + xlab("Sepal Width")


# Histogram
ggplot(data=iris, aes(x=Sepal.Width, group=Species, fill=Species)) + 
  geom_histogram(position="stack") + theme(legend.position=c(.8, .8)) + xlab("Sepal Width")
...
ggplot(data=lineupdata, aes(x=Sepal.Width, y=Petal.Width)) + 
  geom_jitter() + facet_wrap(~.sample) + theme(axis.text=element_blank(), axis.title=element_blank())
@
