...
Finally, complex graphs often facilitate different types of participant tasks; rather than simple numerical judgments or information lookup, complex graphs may encourage (or require) viewers to use prior knowledge and interpretation skills. These additional complications make experimental study of complex or domain-specific graphs more difficult. Many of these problems (types of complexity, expanded tasks, prior knowledge) make further work in this area somewhat more difficult. One concept that facilitates such work in statistical graphics is the \emph{grammar of graphics}, which we discuss in the next section. 

\subsubsection{The Grammar of Graphics and Testing Statistical Graphs}
