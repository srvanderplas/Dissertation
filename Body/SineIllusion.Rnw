...
source("../Code/sineIllusion/functions.r")
@
...
#source("../Data/sineIllusion/NewOzoneData.R")
datasub <- read.csv("../Data/sineIllusion/Ozone-subset.csv")
...
cairo_pdf("../Figure/sineIllusion/fig-original-redo.pdf", width=5, height=5)
...
\begin{figure}[h!tbp]
\centering
\includegraphics[keepaspectratio=TRUE,width=.7\textwidth]{Images/sineIllusion/PlayfairExportImports}
...
datasub <- read.csv("../Data/sineIllusion/Ozone/Ozone-subset.csv")
...
cairo_pdf("../Figure/sineIllusion/fig-CtsQuadratic.pdf", width=6, height=4)
...
\includegraphics[width=.8\textwidth]{../Images/sineIllusion/shiny.pdf}
...
trial.sum <- read.csv("../Data/sineIllusion/SummaryTable.csv", row.names=1, stringsAsFactors=FALSE)
trial.sequence <- read.csv("../Data/sineIllusion/IndivTrajectory.csv", row.names=1, stringsAsFactors=FALSE)
library(lubridate)
...
if (!file.exists("../Data/sineIllusion/modelsim.RData")) {
  # summary(model)
  N <- 1000
  ...
  save(res, file="../Data/sineIllusion/modelsim.RData")
} else {
  load("../Data/sineIllusion/modelsim.RData")
}
...
print(fixeftab, include.rownames=FALSE, file="../Figure/sineIllusion/fixef.txt", floating=FALSE)
...
print(ranefCIstab, include.rownames=FALSE, file="../Figure/sineIllusion/ranef.txt", floating=FALSE)
@
...
gasprices <- read.csv("../Data/sineIllusion/GasPrices.csv", stringsAsFactors=F)
...
save.image("../Data/sineIllusion/gasprices.RData")
@
<<gaspricesCorrectionPlots,echo=F,include=F,cache=T>>=
load("../Data/sineIllusion/gasprices.RData")
...
ggsave(file="../Figure/sineIllusion/gas-x-corrected.pdf", width=8, height=4)
...
ggplot(data=gas.cor.y) + 
  ...
  theme_bw() + geom_point(aes(x=1995, y=0), size=0.1) + 
  theme(plot.margin=unit(c(0,0,0,0), "cm"))
ggsave(file="../Figure/sineIllusion/gas-y-corrected.pdf", width=8, height=4)
