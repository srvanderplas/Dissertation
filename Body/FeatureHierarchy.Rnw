...
<<FeatureHierarchy-setup, fig.keep='all', cache=FALSE, echo=FALSE, eval=TRUE, include=FALSE>>=
rm(list=ls())
...
if(!"Body" %in% list.files()){
  opts_chunk$set(fig.path='Figure/FeatureHierarchy/fig-', cache.path='cache/', fig.align='center', fig.width=5, fig.height=5, fig.show='hold', par=TRUE, cache=TRUE, concordance=TRUE, autodep=TRUE, root.dir="../", message=F, warning=F, error=F)
  datadir <- "../Data/FeatureHierarchy/"
  imgdir <- "../Figure/FeatureHierarchy/"
  codedir <- "../Code/FeatureHierarchy/"
} else {
  opts_chunk$set(fig.path='Figure/FeatureHierarchy/fig-', cache.path='cache/', fig.align='center', fig.width=5, fig.height=5, fig.show='hold', par=TRUE, cache=TRUE, concordance=TRUE, autodep=TRUE, message=F, warning=F, error=F)
  datadir <- "Data/FeatureHierarchy/"
  imgdir <- "Figure/FeatureHierarchy/"
  codedir <- "Code/FeatureHierarchy/"
}
...
if (file.exists(paste0(datadir, "nulldist.Rdata"))) {
  load(paste0(datadir, "nulldist.Rdata"))
} else {
...
# res <- nulldist(N=N, sC=sC, sT=sT)
  ...
  save(res, file=paste0(datadir, "nulldist.Rdata"))
}
...
qplot(data=subset(longres, type!="gini"), x=value, y=..density.., stat="density", color=dist, fill=dist, geom="area", alpha=I(.5), 
...
<<simulationparameters-setup,echo=F,include=F, fig.width=10, fig.height=6.5, out.width='.8\\linewidth'>>=
load(paste0(datadir, "SimulationDatasetCriteriaTurk16.Rdata"))
...
<<simulationparameters-1,echo=F,include=F, fig.width=10, fig.height=6.5, out.width='.8\\linewidth'>>=
...
\begin{figure}\centering
<<simulationparameters-2,echo=F,include=F, fig.width=10, fig.height=6.5, out.width='.8\\linewidth'>>=
...
users <- read.csv(paste0(datadir, "turk16_users_anon.csv"), stringsAsFactors=F)
...
\begin{figure}\centering
<<group-fixef, echo=FALSE, dependson='group-model', fig.width=8, fig.height=4, out.width='.75\\linewidth'>>=
...
print(xtable(group.fixef2[,c(8, 1:4, 9)], caption=c("Fitted values of fixed effects for the model described in \\eqref{eqn:groupModel}.", "Fixed effects for group target logistic model"), label="tab:group.fixef", align=c('r', 'r', 'r', 'r', 'r', 'r', 'r'), digits=c(0, 0, 4, 4, 2, 4, 0)), include.rownames=F, file=paste0(imgdir, "group-fixef-table"))
@
...
\begin{figure}\centering
<<simulationparameters-3,echo=F,include=F, fig.width=10, fig.height=6.5, out.width='.8\\linewidth',dependson='simulationparameters-setup'>>=
tmp <- subset(dataset.criteria, type=="gini")
tmp$dist <- gsub("Max", "Min", tmp$dist)
qplot(data=tmp, x=LB, xend=UB, y=sd.trend, yend=sd.trend, color=dist, geom="segment") +
  geom_point(aes(x=LB, y=sd.trend, color=dist))  + 
...
\begin{figure}
<<timemodel-plottype, echo=FALSE, fig.width=10, fig.height=5, out.width='\\textwidth', dependson='results-setup'>>=
library(lubridate)
...
qplot(estimate, pos, data=coefs, colour=outcome, size=I(4), shape=outcome) + theme_bw() + 
  ...
  xlab("Estimate of the (additional) time to evaluate each plot type (in seconds) given outcome") + ylab("")
@
...
coefs <- data.frame(exp(confint(time.model2, method="Wald")))[-c(1:11),]
names(coefs) <- c("low", "high")
coefs$estimate <- exp(fixef(time.model2)[-c(1:11)])
coefs$name <- names(fixef(time.model2))[-c(1:11)]
ll <- ldply(strsplit(coefs$name, split=":"), function(x) x)
coefs$outcome <- gsub("simpleoutcome", "", ll$V1)
...
qplot(estimate, pos, data=subset(coefs, outcome!="Both Identified"), colour=trend.diff, size=I(4), shape=trend.diff) + theme_bw() + 
  ...
  scale_y_continuous(breaks=1:2, labels=unique(coefs$k)) + 
  xlab("Estimate of the (additional) time to evaluate each plot type (in seconds) given outcome") + ylab("K")
...
\begin{figure}
\centering
<<participant-confidence,echo=F, fig.width=6.5, fig.height=4.25, out.width='.5\\linewidth'>>=
conf.acc <- modeldata %>% 
