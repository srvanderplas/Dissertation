...
$\sigma_C$ & Scatter around cluster centers & \begin{tabular}{ll} .20, .25, .30 ($K=3$)\\ .15, .20, .25 ($K=5$) \end{tabular}
\\\hline
\end{tabular}
\end{center}
...
The parameter values were chosen after examining the parameter space through simulation of 1000 lineup datasets for each combination of $\sigma_T\in\{0.2, 0.25, ..., 0.5\}$, $\sigma_C\in\{0.1, 0.15, ..., 0.4\}$, and $K\in\{3,5\}$; 
